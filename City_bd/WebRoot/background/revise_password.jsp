 ...
 <div class="wrap">
  ...
  <table class="noborder">
   ...
   <tr>
    ...
    <td><input type="submit" class="tdBtn" value="保存" onclick="submit('${pojo.root}')"/></td>
   </tr>
  </table>
 </div>
</body>
  ...
  if(re.test(textBox)){//如果验证正确执行以下代码
	...
	function submit(root){
		...
		if(email != "" && rootpwd != "" && pwd != "" && pwd1 != ""){
			if(pwd == pwd1){
				$.ajax({
					url:'/City_bd/admin/upadmin',
					type:'get',
					data:{root:root,email:email,rootpwd:rootpwd,pwd:pwd,pwd1:pwd1},
